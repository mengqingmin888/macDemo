...
- (void)setMenuFood:(FoodModel *)menuFood
{
    if (_menuFood != menuFood) {
        [_menuFood release];
        _menuFood = [menuFood retain];
    }
    //菜名
    _nameLabel.text = menuFood.name;
    //收藏数量
    _clickCountLabel.text = menuFood.clickCount;
